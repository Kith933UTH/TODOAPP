    ...
    return html`
      <footer class="footer">
        <div class="wrap-footer">
          <div class="count-incomplete">
            ${html`<span> ${(filter !== 'completed') ? (todos.filter(filters.active).length + '</span> item left') : (todos.filter(filters.completed).length + '</span> item completed')}`}
          </div>
          <div class="wrap-filters">
            <ul class="filters">
              ${Object.keys(filters).map((key) => html`              
                <li onclick="dispatch('changeFilter', '${key}')">
                  <a class="filter-item 
